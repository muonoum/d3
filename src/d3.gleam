...
pub fn update(model: Model) -> Model {
  ...
  let object =
    Object(
      ..object,
      world: transform.scale_v3(object.scale)
        |> m4.multiply(transform.rotate_v3(orientation))
...
pub fn points(buffer, box: #(Float, Float, Float, Float), p1, p2, p3) {
  ...
  let smp = V3(x +. 0.5, y +. 0.5, 0.0)

  let w1 = edge(p1, p2, smp)
  use <- bool.guard(w1 <. 0.0, buffer)
  let w1 = w1 /. area

  let w2 = edge(p2, p3, smp)
  use <- bool.guard(w2 <. 0.0, buffer)
  let w2 = w2 /. area

  let w3 = edge(p3, p1, smp)
  use <- bool.guard(w3 <. 0.0, buffer)
  let w3 = w3 /. area

  dict.insert(buffer, #(x, y), #(w1, w2, w3))
}

pub fn edge(a, b, p) {
  let v1 = v3.subtract(p, a)
  let v2 = v3.subtract(b, a)
  // let _ = m2.det(m2.M2(m2.R2(v1.x, v1.y), m2.R2(v2.x, v2.y)))
  // let _ = v3.mag(v3.cross(v1, v2))
  v1.x *. v2.y -. v1.y *. v2.x
}
